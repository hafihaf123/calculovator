...
class Expression1 {
	...
	public static String eval(List<String> numbers, List<String> characters) {
		...
		while (Utility.contains(characters, "*/÷^abcdefghijklmnopqrstuvwxyz")) {
			for (int i = 0; i < lenC; i++) {
				String c = characters.get(i);
				if (!Utility.contains(c, "*/÷^abcdefghijklmnopqrstuvwxyz")) {
					continue;
				}
...
class Utility {
	...
	public static boolean contains(String checking, String x) {
		char[] arr = x.toCharArray();
